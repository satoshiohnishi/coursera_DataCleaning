...
# Group the data frame and assign to new average_by_person by the person_id and calculate the mean of each column
average_by_person <- group_by(df_mean_std_labels, person_id) %>% summarise_all(mean)

# Drop the activity_names column
average_by_person <- average_by_person[, -which(names(average_by_person) == "activity_names")]
print(average_by_person)

# Write the data frame to csv file
write.csv(average_by_person, "average_by_person.csv", row.names = FALSE)

# Group the data frame and assign to new average_by_activity by the activity_names and calculate the mean of each column
average_by_activity <- group_by(df_mean_std_labels, activity_names) %>% summarise_all(mean)

# Drop the person_id column
average_by_activity <- average_by_activity[, -which(names(average_by_activity) == "person_id")]
print(average_by_activity)

# Write the data frame to csv file
write.csv(average_by_activity, "average_by_activity.csv", row.names = FALSE)
